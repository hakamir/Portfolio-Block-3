...
def update_galleries():
    ...
    for gallery in data:
        ...
        if gallery_id:
            galleries_col.update_one(
                {'_id': ObjectId(gallery_id)},
                {'$set': gallery}
            )
        else:
            galleries_col.insert_one(gallery)
    return jsonify({'updated': True}), 200
...
def delete_gallery(id):
    galleries_col.delete_one({'_id': ObjectId(id)})
    return jsonify({'deleted': id}), 200
...
def upload_image():
    ...
    dest = os.path.join(UPLOAD_FOLDER, 'gallery', gallery_slug)
    os.makedirs(dest, exist_ok=True)
    file.save(os.path.join(dest, image_src))
    return jsonify({'uploaded': image_src}), 201

@app.route('/gallery/next-src', methods=['GET'])
@jwt_required()
def get_next_src():
